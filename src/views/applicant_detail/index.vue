<template>
  <div class="applicant-detail-container" v-if="applicantData">
    <div class="header">
      ...
      <div class="action-buttons">
        <button
          v-if="applicantData.status === 'SUBMITTED'"
          @click="shortlistApplicant(applicantData._id)"
          :disabled="isShortlisting || isHiring || isRejecting || isProcessing"
          class="btn-shortlist"
        >
        ...
        <button
          v-if="['SHORTLISTED'].includes(applicantData.status)"
          @click="openScheduleModal(applicantData._id)"
          :disabled="isHiring || isRejecting || isShortlisting || isProcessing"
          class="btn-interview"
        >
        ...
        <button
          v-if="applicantData.status === 'INTERVIEWING'"
          @click="hireApplicant(applicantData._id)"
          :disabled="isHiring || isRejecting || isShortlisting || isProcessing"
          class="btn-hired"
        >
        ...
        <button
          v-if="
            applicantData.status === 'INTERVIEWING' ||
            applicantData.status === 'SUBMITTED'
          "
          @click="rejectApplicant(applicantData._id)"
          :disabled="isRejecting || isHiring || isShortlisting || isProcessing"
          class="btn-reject"
        >
    ...
    <section class="card personal-info">
      ...
      <div class="detail-grid">
        <p>
          <strong>Full Name:</strong> {{ applicantData.applicant.first_name }}
          {{ applicantData.applicant.last_name }}
        </p>
        <p><strong>Gender:</strong> {{ applicantData.applicant.sex }}</p>
        <p>
          <strong>Date of Birth:</strong> {{ formatDate(applicantData.applicant.dob) }}
        </p>
        <p>
          <strong>Current Address:</strong> {{ applicantData.applicant.current_address }}
        </p>
        ...
        <p>
          <strong>Applied Count:</strong> {{ applicantData.applicant.apply_count }} times
        </p>
      </div>
    </section>
    ...
    <section class="card application-details">
      ...
      <div class="detail-grid">
        <p>
          ...
          <span class="highlight-position">{{ applicantData.apply_position }}</span>
        </p>
        <p>
          <strong>Job Title (Internal):</strong>
          {{ applicantData.jobId?.title?.des_en }} ({{
            applicantData.jobId?.department?.name_en
          }}
          Dept.)
        </p>
        <p>
          <strong>Job Salary Range:</strong>
          <span class="salary-range">${{ applicantData.jobId?.salary }}</span>
        </p>
        <p>
          <strong>Requested Location:</strong> {{ applicantData?.requested_location }}
        </p>
        <p>
          <strong>Expected Salary:</strong>
          <span class="salary-range">${{ applicantData.expected_salary }}</span>
        </p>
        <p><strong>Applied Date:</strong> {{ formatDate(applicantData.createdAt) }}</p>
        <p class="full-width">
          <strong>Reason for Application:</strong> {{ applicantData.why_apply }}
        </p>
        <p>
          <strong>Knows Someone:</strong> {{ applicantData.knows_someone ? "Yes" : "No" }}
        </p>
        <p
          v-if="
            applicantData.knows_someone && applicantData.knows_someone_details !== 'none'
          "
        >
          <strong>Details:</strong> {{ applicantData.knows_someone_details }}
        </p>
        <a
          v-if="applicantData?.resume?._id"
          @click="viewResume(applicantData.resume._id, applicantData.resume.filename)"
          target="_blank"
          class="resume-link fa-i-cursor"
        >
    ...
    <section
      class="card interview-details"
      v-if="applicantData.interview && applicantData.interview.date"
    >
      ...
      <div class="detail-grid">
        ...
        <p class="full-width">
          <strong>Notes:</strong> {{ applicantData.interview.notes || "N/A" }}
        </p>
      </div>
    </section>
    <section class="card education">
      ...
      <div class="detail-grid">
        ...
        <p>
          <strong>Attended:</strong> {{ applicantData.education_from_year }} to
          {{ formatDateYear(applicantData.education_to_year) }}
        </p>
      </div>
    </section>
    ...
    <section class="card experience">
      ...
      <div class="detail-grid">
        ...
        <p>
          ...
          <span class="salary-range">${{ applicantData.salary_usd }}</span>
        </p>
        <p>
          <strong>Duration:</strong> {{ formatDate(applicantData.start_date) }} to
        ...
        <form @submit.prevent="scheduleInterview">
          ...
          <div class="form-group">
            ...
            <input
              ...
              required
              :placeholder="
                interviewDetails.mode === 'In-person' ? 'Office Address' : 'Meeting Link'
              "
              class="form-input"
            />
          </div>
    ...
    <div class="modal-content modal-dialog-small">
      ...
      <div class="modal-footer">
        ...
        <button
          ...
          :class="['btn-primary', confirmDialog.styleClass]"
        >
  ...
  <div
    ...
    :class="[
      'snackbar',
      {
        'snackbar-success': snackbar.type === 'success',
        'snackbar-error': snackbar.type === 'error',
      },
    ]"
  >
    <i
      :class="[
        'icon-left',
        snackbar.type === 'success' ? 'fas fa-check-circle' : 'fas fa-times-circle',
      ]"
    ></i>
    {{ snackbar.message }}
  </div>
...
<script>
import { getApplicantById, updateInterviewStatus, getOneResume } from '@/apis/applicant';
...
export default {
  ...
  methods: {
    ...
    async updateStatus(
      ...
      try {
        ...
        if (res.status === 1) {
          ...
          this.showSnackbar(
            `Failed to perform action: ${actionName}. ${res.message}`,
            'error'
          );
          return false;
        }
        ...
        const errorMessage =
          apiErrorMessage ||
          err.message ||
          `An unexpected network error occurred during ${actionName.toLowerCase()}.`;
    ...
    async viewResume(resumeId, fileName) {
      try {
        this.isLoading = true;
        debugger;
        const response = await getOneResume(resumeId);

        // Handle if your request() returns the full Axios response or just the data
        ...
        this.showSnackbar(`Opening ${fileName || 'resume'}`, 'success');

        setTimeout(() => URL.revokeObjectURL(url), 30000);
      } catch (error) {
        console.error('View resume failed:', error);
        this.showSnackbar(`Failed to view resume: ${error.message}`, 'error');
      } finally {
        this.isLoading = false;
      }
    }
