  ...
  <div class="min-h-screen bg-gray-50 flex flex-col">
    <div class="w-full">
          ...
          <table class="min-w-full table-auto text-sm">
            ...
            <tbody class="text-gray-700">
              ...
              >
                ...
                <td class="px-4 py-2">
                  {{ applicant.applicant.gender || 'N/A' }}
                </td>
                <td class="px-4 py-2">
      ...
      <transition name="modal">
        ...
        >
          <div
            class="bg-white rounded-lg shadow-md p-4 w-full max-w-2xl mx-4 transform transition-all"
          >
            <div
              class="flex justify-between items-center mb-3 border-b border-gray-200 pb-2"
            >
            ...
            <div class="grid gap-4 py-4">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                  ...
                  >
                  <p
                    class="mt-1 text-gray-900 bg-white border border-gray-300 rounded px-3 py-2"
                  >
                ...
                <div class="space-y-2 col-span-2">
                  ...
                  >
                  <p
                    class="mt-1 text-gray-900 bg-white border border-gray-300 rounded px-3 py-2 truncate"
                  >
                    <button
                      v-if="selectedApplicant?.resume?.fileName"
                      @click="
                        downloadResume(
                    ...
                    >
                      <i
                        v-if="!loading[selectedApplicant._id]"
                        class="fas fa-cloud-download-alt"
                      ></i>
                      <i v-else class="fas fa-spinner fa-spin"></i>
                      {{ selectedApplicant.resume.fileName }}
...
export default {
  ...
  setup() {
    // Use reactive for loading to ensure Vue 3 reactivity
  ...
  computed: {
    filteredApplicants() {
      return this.applicants.filter((applicant) => {
        const fullName = `${applicant.applicant?.first_name || ''} ${
          applicant.applicant?.last_name || ''
        }`.toLowerCase();
        const matchSearch =
          this.searchQuery === '' ||
      ...
      try {
        ...
        if (response.status === 1) {
          this.applicants = response.data.map((applicant) => {
            this.loading[applicant._id] = false;
            return {
              ...applicant,
              job: {
                title:
          ...
          this.totalApplicants = response.pagination?.total || 0;
          this.showAlert(
            `Successfully fetched ${this.applicants.length} applicants`
          );
          console.log('Mapped applicants:', this.applicants); // Debug mapped data
        } else {
